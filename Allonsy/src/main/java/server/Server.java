...
public class Server {
    ...
    //список коннекторов к ардуино. скписок для того, чтобы можно было поддерживать множество микроконтроллеров
    private static List<ArduinoController> controllers;

    public static void main(String[] args) {
        users = new LinkedList<>();
        controllers = new ArrayList<>();
        controllers.add(new ArduinoController());

        Waiter waiter = new Waiter();
        waiter.start();
        System.out.println("Запустили приём подключений"); //todo logs
    }
